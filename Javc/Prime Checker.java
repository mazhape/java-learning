...
checkPrime(int... numbers): This method accepts a variable number of integer arguments. It iterates over each number, checks if its prime using the isPrime method, and prints it if it is.
isPrime(int number): A helper method to determine if a number is prime. It returns true if the number is prime and false otherwise.
Main Method:

Reads five integers from standard input using BufferedReader.
Creates an instance of the Prime class.
Calls the checkPrime method with different sets of integers.
Checks for method overloading by ensuring that each method name in the Prime class is unique.
This solution ensures that only prime numbers are printed for each input scenario, and it avoids method overloading by using a single checkPrime method that handles multiple arguments.
